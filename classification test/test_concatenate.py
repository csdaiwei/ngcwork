...
if len(sys.argv) != 2:
    print 'usage: $ python test_concatenate.py [picklefilename]'
    print 'as     $ python test_concatenate.py hhl_mb_6498.pickle\n'
    sys.exit()
...
models = [LogisticRegression, RandomForestClassifier, AdaBoostClassifier, GradientBoostingClassifier]

resultlog = {}
for (g, gn) in zip(groups, groupnames):
    resultlog[gn] = {} 
    for m in models:
        resultlog[gn][m.__name__] = []
...
for (g, gn, dim) in zip(groups, groupnames, groupdims):
    ...
    for epoch in range(0, REPEAT):
        ...
        for m in models:
            ...
            resultlog[gn][m.__name__].append([auc, acc, tp, fp, tn, fn])

            #print '      auc:%.4f accu:%.4f tp:%d fp:%d tn:%d fn:%d, time:%4f, %s'%(auc, acc, tp, fp, tn, fn, time() - start, m.__name__)
...
for (g, gn) in zip(groups, groupnames):
    ...
    for m in models:
        print '  %s '%m.__name__[0:4] ,
        r = np.array(resultlog[gn][m.__name__])
